...
int main(int argc, char *argv[]){
  ...
  for(int thetaId=0;thetaId<Ntheta;++thetaId){
    ...
    for (int r=1;r<size;++r){
    	MPI_Recv(&pixels[0],pixCount*3/size,MPI_UNSIGNED_CHAR,
	r,999,MPI_COMM_WORLD,&status);

    for(int s=0;s<pixCount*3/size;++i){
    	img[pixCount*3-(r*pixCount*3/size+s)]=pixels[pixCount*3/size-1-s];
    }
