...
int main(int argc, char *argv[]){
  ...
  unsigned char *c_img = (unsigned char*) calloc(3*WIDTH*HEIGHT, sizeof(char));
  unsigned char c_img = &img;
  ...
  for(int thetaId=0;thetaId<Ntheta;++thetaId){
    ...
    cudaEventElapsedTime(float* ms,start,end);
    ms=ms/1000;
    elapsed+=ms;
    ...
    for(int subStep=0;subStep<NsubSteps;++subStep){
      ...
      gridPopulate(grid, scene->Nshapes, shapes);
	cudaMalloc(&c_shapes, Nshapes);
	cudaMemcpy(shapes, c_shapes, Nshapes, cudaMemcpyDeviceToHost);
	cudaMalloc(&c_bboxes, boxCount);
	cudaMemcpy(bboxes, c_bboxes, boxCount, cudaMemcpyDeviceToHost);
    }
