...
void gridPopulate(grid_t *grid, int Nshapes, shape_t *shapes){
  ...
  free(boxCounts);
  free(boxCounters);
  cudaMalloc(&c_lights, Nlights);
  cudaMemcpy(lights, c_lights, Nlights, cudaMemcpyDeviceToHost);
  cudaMalloc(&c_shapes, Nshapes);
  cudaMemcpy(shapes, c_shapes, Nshapes, cudaMemcpyDeviceToHost);
  cudaMalloc(&c_materials, Nmaterials);
  cudaMemcpy(materials, c_materials, Nmaterials, cudaMemcpyDeviceToHost);
